...
class JobsMarker
{
    ...
    public function markJobAsFinished(Job $job, array $info)
    {
        // If this Job is a retry of another one, mark also the retried as finished
        if ($job->isRetry()) {
            //\Symfony\Component\VarDumper\VarDumper::dump('[JobsMarker::markAsFinished] ('.$job->getId().') before marking parents as retry_finished: ' . round(memory_get_usage(true) / 1000000, 2));
            $this->markParentsAsRetryFinished($job->getRetryOf());
            //\Symfony\Component\VarDumper\VarDumper::dump('[JobsMarker::markAsFinished] ('.$job->getId().') after marking parents as retry_finished: ' . round(memory_get_usage(true) / 1000000, 2));
        }
